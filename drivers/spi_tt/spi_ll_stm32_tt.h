...
#ifndef ZEPHYR_DRIVERS_SPI_SPI_LL_STM32_TT_H_
#define ZEPHYR_DRIVERS_SPI_SPI_LL_STM32_TT_H_
...
struct spi_stm32_data {
	...
	/* Timer-triggered DMA mode */
	bool timer_dma_active;
	const struct spi_buf *timer_tx_buf;
	const struct spi_buf *timer_rx_buf;
...
/* Timer-triggered DMA API functions */
/**
 * @brief Setup timer-triggered DMA for continuous SPI operation
 * 
 * Configures the SPI to use timer-triggered DMA instead of normal SPI transfers.
 * The timer trigger is configured in devicetree (e.g., dmas = <&dma2 2 TIM1_UP>).
 * 
 * @param dev SPI device
 * @param config SPI configuration 
 * @param tx_buf SPI buffer containing commands to transmit (encoder commands)
 * @param rx_buf SPI buffer to receive data (encoder responses)  
 * @return 0 on success, negative error code on failure
 */
int spi_stm32_setup_timer_dma(const struct device *dev,
                              const struct spi_config *config,
                              const struct spi_buf *tx_buf,
                              const struct spi_buf *rx_buf);

/**
 * @brief Start timer-triggered DMA operation
 * 
 * Enables the timer-triggered DMA transfers. The PWM timer will now
 * trigger SPI transfers automatically.
 * 
 * @param dev SPI device
 * @return 0 on success, negative error code on failure
 */
...
/**
 * @brief Stop timer-triggered DMA operation
 * 
 * Disables timer-triggered DMA and returns SPI to normal operation mode.
 * 
 * @param dev SPI device
 * @return 0 on success, negative error code on failure
 */
...
/**
 * @brief Check if timer-triggered DMA is active
 * 
 * @param dev SPI device
 * @return true if timer DMA is active, false otherwise
 */
bool spi_stm32_is_timer_dma_active(const struct device *dev);

#endif	/* ZEPHYR_DRIVERS_SPI_SPI_LL_STM32_TT_H_ */
