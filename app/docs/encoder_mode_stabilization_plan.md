...
- Require:
  ...
  - Encoder pipeline is not busy during mode entry.
...
- Add a shell status command showing the readiness gate and relevant encoder pipeline/protocol counters.
...
- Fixed AEAT-9955 SPI4-8 register reads to account for the device response pipeline on register accesses.
...
- Cleared pipeline counters after a successful mapping run because the mapping result preserves the rejected/error counts.
...
- Allowed encoder-to-encoder transitions while the realtime encoder pipeline is active; these transitions skip unsafe register rereads and treat historical counters as diagnostics.
...
- HIL: generated-sweep mapping passed with `dir=-1`, correlation about `-0.982`, offset about `-1.59 deg mechanical`, and no final-pass pipeline errors.
- HIL: `current_encoder -> velocity_encoder -> position_encoder` transitions completed without faults.
- HIL: `position_encoder` hold settled at approximately zero Iq and encoder pipeline counters showed no transport/frame/parity/CRC/status/glitch errors in the final pass.

## Remaining Observations

- Occasional isolated AEAT CRC errors still appear during long sweeps on some runs. The control path did not fault on isolated events, and the pipeline counters expose them for diagnosis.
