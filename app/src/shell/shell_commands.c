...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_chopper_calib,
	...
	SHELL_CMD(status, NULL, "Show edge capture and derived slot/tooth center status", cmd_motor_chopper_calib_status),
	SHELL_CMD(apply, NULL, "Apply derived slot/tooth centers to profile sequence points", cmd_motor_chopper_calib_apply),
	SHELL_CMD(clear, NULL, "Clear calibration buffers and edge map", cmd_motor_chopper_calib_clear),
	SHELL_SUBCMD_SET_END
);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_chopper,
	...
	SHELL_CMD_ARG(sensor, NULL, "Get/set photo-interrupter emitter and report PB4 blade output [0|1]", cmd_motor_chopper_sensor, 1, 1),
	SHELL_CMD(calib, &sub_motor_chopper_calib, "Chopper edge/center calibration", NULL),
	SHELL_SUBCMD_SET_END
);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_commission_encoder,
	SHELL_CMD_ARG(run, NULL,
		      "Stage one-way generated-sweep commutation map <current_a> <velocity_hz> <cycles>",
		      cmd_motor_commission_encoder_run, 4, 0),
	SHELL_CMD_ARG(robust, NULL,
		      "Stage robust commutation map <current_a> <velocity_hz> <cycles> [bidirectional]",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_commission_detent,
	SHELL_CMD_ARG(run, NULL,
		      "Run detent feedforward capture <velocity_hz> <cycles> [decimation] [iq_limit_a]",
	...
	SHELL_CMD_ARG(validate, NULL,
		      "Compare low-speed velocity ripple with detent off/on <velocity_hz> <duration_ms>",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_commission_ripple,
	SHELL_CMD_ARG(run, NULL,
		      "Run electrical-angle ripple capture <velocity_hz> <duration_ms> [decimation] [iq_limit_a]",
	...
	SHELL_CMD_ARG(validate, NULL,
		      "Compare low-speed velocity ripple with electrical ripple off/on <velocity_hz> <duration_ms>",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_commission_electrical,
	...
	SHELL_CMD_ARG(run, NULL,
		      "Run production electrical-ID sequence [rs_current_a] [demod_pulse_v] [samples]",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_commission,
	SHELL_CMD_ARG(run, NULL, "Run baseline electrical + encoder commissioning [slow|confirm] [apply]",
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_motor_settings,
	...
	SHELL_CMD(save, NULL,
		  "Save settings [model electrical|model encoder|identity|limits|controllers|detent|all]",
		  cmd_motor_settings_save),
	SHELL_CMD(load, NULL,
		  "Load settings [model electrical|model encoder|identity|limits|controllers|detent|all]",
		  cmd_motor_settings_load),
	SHELL_CMD(clear, NULL,
		  "Clear settings [model electrical|model encoder|identity|limits|controllers|detent|all]",
