...
/* This application supports only the low-latency encoder_rt path. */
#if DT_NODE_HAS_COMPAT(DT_ALIAS(encoder1), brcm_aeat_9955_fast) || \
	DT_NODE_HAS_COMPAT(DT_ALIAS(encoder1), magntek_mt6835_fast)
#include <drivers/encoder_rt.h>

static const struct device *const motor_encoder_rt_dev = DEVICE_DT_GET(DT_ALIAS(encoder1));
#else
#error "encoder1 must use a fast encoder_rt-compatible driver"
#endif
...
static int motor_encoder_acquisition_collect_encoder_rt(struct motor_encoder_sample *sample)
{
...
int motor_encoder_acquisition_collect(struct motor_encoder_sample *sample)
{
	...
	return motor_encoder_acquisition_collect_encoder_rt(sample);
}
