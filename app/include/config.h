...
#define PROFILE_SEQUENCE_TRIGGER_SRC_SOFTWARE 2U
...
struct motor_calibration_ctx {
	...
	uint16_t align_sample_count;      /* Fresh encoder samples accumulated in ISR */
	float32_t align_sum_sin;         /* Circular-mean accumulator for ALIGN sample */
	float32_t align_sum_cos;         /* Circular-mean accumulator for ALIGN sample */
	float32_t align_mech_angle_rad;  /* Circular mean of ALIGN sample window */
};
...
#define MOTOR_FLUX_LINKAGE_VPH_ELEC \
	((float32_t)DT_PROP_OR(MOTOR_PARAMS_NODE, flux_linkage_uvphz, 0) / 1000000.0f)
...
#define MOTOR_INERTIA_KGM2 ((float32_t)DT_PROP(MOTOR_PARAMS_NODE, inertia_mgcm2) / 10000000000.0f)
...
#define COMMISSION_AUTO_VELOCITY_BANDWIDTH_HZ \
	((float32_t)DT_PROP_OR(USER_PARAMS_NODE, commission_auto_velocity_bandwidth_mhz, 20000) / 1000.0f)
...
BUILD_ASSERT(DT_PROP_OR(USER_PARAMS_NODE, commission_auto_velocity_bandwidth_mhz, 20000) > 0,
	     "commission-auto-velocity-bandwidth-mhz must be positive");
...
/* 20 kHz current loop, 2 kHz velocity loop, 200 Hz position loop. */
#define VELOCITY_LOOP_DECIMATION_DEFAULT 10U
#define POSITION_LOOP_DECIMATION_DEFAULT 100U
...
#if defined(CONFIG_MOTOR_OUTER_LOOP_MPR_RUNTIME_DEFAULT)
#define OUTER_LOOP_MPR_DEFAULT_ENABLED IS_ENABLED(CONFIG_MOTOR_OUTER_LOOP_MPR_RUNTIME_DEFAULT)
#else
#define OUTER_LOOP_MPR_DEFAULT_ENABLED 0
#endif
#if defined(CONFIG_MOTOR_VELOCITY_DOB_RUNTIME_DEFAULT)
#define VELOCITY_DOB_DEFAULT_ENABLED IS_ENABLED(CONFIG_MOTOR_VELOCITY_DOB_RUNTIME_DEFAULT)
#else
#define VELOCITY_DOB_DEFAULT_ENABLED 0
#endif
