...
static inline int motor_transforms_park(float32_t ia_a,
					float32_t ib_a,
					float32_t elec_angle_rad,
					float32_t *id_a,
					float32_t *iq_a)
{
	if (id_a == NULL || iq_a == NULL) {
		return -EINVAL;
	}
...
static inline int motor_transforms_inv_park(float32_t vd_v,
					    float32_t vq_v,
					    float32_t elec_angle_rad,
					    float32_t *va_v,
					    float32_t *vb_v)
{
	if (va_v == NULL || vb_v == NULL) {
		return -EINVAL;
	}
