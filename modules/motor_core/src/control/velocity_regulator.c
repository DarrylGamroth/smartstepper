				  ...
				  float32_t *iq_cmd_a_out)
{
	...
	float32_t i_next = state->integrator_a + (cfg->ki_a_per_rad * speed_error_rad_s * dt_s);
	i_next = clampf(i_next, -cfg->integrator_limit_a, cfg->integrator_limit_a);
	state->integrator_a = i_next;

	float32_t iq_cmd = (cfg->kp_a_per_rad_s * speed_error_rad_s) + state->integrator_a;
